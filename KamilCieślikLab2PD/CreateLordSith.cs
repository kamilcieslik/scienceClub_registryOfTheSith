...
namespace KamilCieślikLab2PD
{
    public partial class CreateLordSith : Form
    {
        ...
        {
            InitializeComponent();
            ListOfLordSith = listOfLordSith;
            pictureBoxSithImage.Image = Properties.Resources.defaultImage;
        }

        public List<LordSith> ListOfLordSith { get; set; }
...
Pamiętaj, że rok śmierci nie może być większy niż aktualny, ani mniejszy od roku urodzenia!");
                        ...
                        else
                        {
                            ...
                            if ((textBoxAmountOfKilledJedi.Text == "") || ((textBoxAmountOfKilledJedi.Text != "") && (int.Parse(textBoxAmountOfKilledJedi.Text) == 0)))
                            {
                                ...
                                var lordSith = new LordSith(textBoxName.Text, comboBoxPlanet.Text, textBoxPath.Text, int.Parse(textBoxYearOfBirth.Text), checkBoxWasJedi.Checked, comboBoxColourOfLightsaber.Text, comboBoxLightSaberCombat.Text, 0, int.Parse(textBoxYearOfDeath.Text), comboBoxSpecialPower.Text, int.Parse(textBoxNumberOfHolocrones.Text), comboBoxKindOfPossesedArmy.Text);
                                ListOfLordSith.Add(lordSith);
                                Close();
                            }
                            else
                            {
                                var lordSith = new LordSith(textBoxName.Text, comboBoxPlanet.Text, textBoxPath.Text, int.Parse(textBoxYearOfBirth.Text), checkBoxWasJedi.Checked, comboBoxColourOfLightsaber.Text, comboBoxLightSaberCombat.Text, int.Parse(textBoxAmountOfKilledJedi.Text), int.Parse(textBoxYearOfDeath.Text), comboBoxSpecialPower.Text, int.Parse(textBoxNumberOfHolocrones.Text), comboBoxKindOfPossesedArmy.Text);
                                ListOfLordSith.Add(lordSith);
                                Close();
                            }
                        }
                        ...
                        if ((textBoxAmountOfKilledJedi.Text == "") || ((textBoxAmountOfKilledJedi.Text != "") && (int.Parse(textBoxAmountOfKilledJedi.Text) == 0)))
                        {
                            if (
                                MessageBox.Show(@"Jesteś pewny/a, że ten Sith nie ma żadnego zabójstwa Jedi na koncie?",
                                    @"Pytanie kontrolne!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) !=
                                DialogResult.Yes) return;
                            var lordSith = new LordSith(textBoxName.Text, comboBoxPlanet.Text, textBoxPath.Text, int.Parse(textBoxYearOfBirth.Text), checkBoxWasJedi.Checked, comboBoxColourOfLightsaber.Text, comboBoxLightSaberCombat.Text, 0, comboBoxSpecialPower.Text, int.Parse(textBoxNumberOfHolocrones.Text), comboBoxKindOfPossesedArmy.Text);
                            ListOfLordSith.Add(lordSith);
                            Close();
                        }
                        else
                        {
                            var lordSith = new LordSith(textBoxName.Text, comboBoxPlanet.Text, textBoxPath.Text, int.Parse(textBoxYearOfBirth.Text), checkBoxWasJedi.Checked, comboBoxColourOfLightsaber.Text, comboBoxLightSaberCombat.Text, int.Parse(textBoxAmountOfKilledJedi.Text), comboBoxSpecialPower.Text, int.Parse(textBoxNumberOfHolocrones.Text), comboBoxKindOfPossesedArmy.Text);
                            ListOfLordSith.Add(lordSith);
                            Close();
                        }
                    }
